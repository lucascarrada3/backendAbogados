...
const oAuth2ClientBase = require('../google/googleAuth');
...
exports.saveTokens = async (req, res) => {
  ...
  try {
    const { tokens } = await oAuth2ClientBase.getToken(code);

    const usuario = await db.Usuario.findByPk(idUsuario);
    usuario.googleTokens = JSON.stringify(tokens);
    await usuario.save();
...
exports.createEvent = async (req, res) => {
  ...
  try {
    ...
    const oAuth2Client = oAuth2ClientBase;
